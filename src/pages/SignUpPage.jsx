...
const SignUpPage = () => {
  ...
  const { signup, login, loading, error } = useUserStore();

  const [isLogin, setIsLogin] = useState(false); // A state to toggle between login and signup

  const handleSubmit = (e) => {
    e.preventDefault();
    if (isLogin) {
      // If in login mode, trigger the login function
      login(formData);
    } else {
      // If in signup mode, trigger the signup function
      signup(formData);
    }
  };
            ...
            {isLogin ? (
              <>
                ...
                <span
                  ...
                  className="font-medium text-emerald-400 cursor-pointer hover:text-emerald-300"
                >
